...
def validate_sql(sql: str, emp_no: int, is_manager: bool) -> tuple[bool, str]:
    ...
    # For employees, check they're not referencing another emp_no directly
    if not is_manager:
...
def show_chat(session):
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Logout"):
            ...
            for key in ["session_id", "emp_no", "role", "email", "is_admin"]:
                st.session_state.pop(key, None)
            st.rerun()

    st.markdown("---")

    # FIX: Only initialise messages once per session — never overwrite on rerun
    ...
    if prompt := st.chat_input("Ask a question about HR data..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    # Removed is_data_question() — saves one Groq API call per message.
                    # nl_to_sql handles non-HR questions gracefully via its system prompt.
                        ...
                        if not is_safe:
                            ...
                            try:
                                ...
                                if df.empty:
                                    ...
                                    result_msg = f"Here are the results ({len(df)} row{'s' if len(df) != 1 else ''}):"
